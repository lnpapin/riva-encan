...
async function envoyerNotifications(lot_id, lot_name, lot_num, new_amount, new_bidder) {
  try {
    // Récupérer tous les participants surpassés avec un courriel
...
module.exports = async function handler(req, res) {
  ...
  if (!lot_id || !name || !amount) {
    return res.status(400).json({ error: 'Champs manquants : lot_id, name, amount' });
  }
  ...
  // Mettre à jour le montant et statut du lot
  ...
  // Envoyer les notifications (directement dans cette fonction)
